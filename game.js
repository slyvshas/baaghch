    ...
    setTimeout(() => {
        ...
        if (possibleMoves.length > 0) {
            switch (botDifficulty) {
                case 'easy':
                    // Easy: picks from top 50% of moves (still makes decent moves)
                    selectedMove = possibleMoves[Math.floor(Math.random() * Math.max(1, Math.floor(possibleMoves.length * 0.5)))];
                    break;
                case 'medium':
                    // Medium: picks from top 20% of moves (strong player)
                    selectedMove = possibleMoves[Math.floor(Math.random() * Math.max(1, Math.floor(possibleMoves.length * 0.2)))];
                    break;
                case 'hard':
                    // Hard: always picks the absolute best move
...
function updateStatus() {
    ...
    // Update background color based on turn
    if (turn === "goat") {
        turnIndicator.style.background = "linear-gradient(145deg, #ff9800, #f57c00)"; // Orange for goats
    } else {
        turnIndicator.style.background = "linear-gradient(145deg, #e74c3c, #c0392b)"; // Red for tigers
    }
